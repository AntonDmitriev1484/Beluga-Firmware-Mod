...
#if defined(CONFIG_DWT_API_ERROR_CHECK)
...
typedef struct {
    ...
    uint8 init_xtrim;  // initial XTAL trim value read from OTP (or defaulted to
...
static dwt_local_data_t
    dw1000local[DWT_NUM_DW_DEV]; // Static local device data, can be an array to
                                 // support multiple DW1000 testing
                                 // applications/platforms
static dwt_local_data_t *pdw1000local =
    dw1000local; // Static local data structure pointer
...
int dwt_setlocaldataptr(unsigned int index) {
    ...
    if (DWT_NUM_DW_DEV >
        index) // return error if index outside the array bounds
    {
        return DWT_ERROR;
    }
...
int dwt_initialise(uint16 config) {
    ...
    if (DWT_DEVICE_ID !=
        dwt_readdevid()) // MP IC ONLY (i.e. DW1000) FOR THIS CODE
    {
        return DWT_ERROR;
    }
    ...
    if (!pdw1000local->init_xtrim) // A value of 0 means that the crystal has
                                   // not been trimmed
    {
        pdw1000local->init_xtrim =
            FS_XTALT_MIDRANGE; // Set to mid-range if no calibration value
                               // inside
    }
    // Configure XTAL trim
 ...
 */
uint32 dwt_getpartid(void) { return pdw1000local->partID; }
 ...
 */
uint32 dwt_getlotid(void) { return pdw1000local->lotID; }
...
void dwt_configure(dwt_config_t *config) {
    ...
    // SFD after its configuration or reconfiguration. This issue is not
    // documented at the time of writing this code. It should be in next release
...
int dwt_writetxdata(uint16 txFrameLength, uint8 *txFrameBytes,
    ...
    assert((pdw1000local->longFrames && (txFrameLength <= 1023)) ||
           (txFrameLength <= 127));
...
int32 dwt_readcarrierintegrator(void) {
    ...
    dwt_readfromdevice(DRX_CONF_ID, DRX_CARRIER_INT_OFFSET, DRX_CARRIER_INT_LEN,
                       buffer);

    for (j = 2; j >= 0;
         j--) // arrenge the three bytes into an unsigned interger value
    {
...
uint32 dwt_readtxtimestamplo32(void) {
    ...
    return dwt_read32bitreg(TX_TIME_ID); // Read TX TIME as a 32-bit register to
                                         // get the 4 lower bytes out of 5
}
...
uint32 dwt_readrxtimestamplo32(void) {
    ...
    return dwt_read32bitreg(RX_TIME_ID); // Read RX TIME as a 32-bit register to
                                         // get the 4 lower bytes out of 5
}
 ...
 * no return value
 */
void dwt_writetodevice(uint16 recordNumber, uint16 index, uint32 length,
                       const uint8 *buffer) {
    ...
    if (index == 0) // For index of 0, no sub-index is required
    {
        ...
        assert(
            (index <= 0x7FFF) &&
            ((index + length) <=
             0x7FFF)); // Index and sub-addressable area are limited to 15-bits.
#endif
        header[cnt++] =
            0xC0 |
            recordNumber; // Bit-7 is WRITE operation, bit-6 one=sub-address
                          ...
                          // byte is required
        {
            ...
            header[cnt++] =
                0x80 | (uint8)(index); // Bit-7 one means extended index, bits
                                       // 6-0 is low seven bits of index.
            header[cnt++] =
 ...
 * no return value
 */
void dwt_readfromdevice(uint16 recordNumber, uint16 index, uint32 length,
                        uint8 *buffer) {
...
void dwt_configuresleepcnt(uint16 sleepcnt) {
    ...
    dwt_write8bitoffsetreg(AON_ID, AON_CFG0_OFFSET,
                           0x00); // NB: this write change the default LPCLKDIVA
                                  // value which is not used anyway.
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, 0x00);
...
int dwt_spicswakeup(uint8 *buff, uint16 length) {
        ...
        DWT_DEVICE_ID) // Device was in deep sleep (the first read fails)
    {
        ...
        dwt_readfromdevice(0x0, 0x0, length,
                           buff); // Do a long read to wake up the chip (hold
                                  // the chip select low)
...
void dwt_setrxaftertxdelay(uint32 rxDelayTime) {
    ...
    val |= (rxDelayTime &
            ACK_RESP_T_W4R_TIM_MASK); // In UWB microseconds (e.g. turn the
...
void dwt_isr(void) {
    ...
    if (status & SYS_STATUS_RXFCG) {
        ...
        if ((status & SYS_STATUS_AAT) &&
            ...
            pdw1000local->cbData.status &=
                ~SYS_STATUS_AAT; // Clear AAT status bit in callback data
                                 // register copy
            pdw1000local->wait4resp = 0;
        }
    ...
    if (status & SYS_STATUS_TXFRS) {
        ...
        // set) that we receive through using wait4resp to a previous TX (and
        // assuming that the IRQ processing of that TX has already been
        // handled), then we need to handle the IC issue which turns on the RX
        // again in this situation (i.e. because it is wrongly applying the
        // wait4resp after the ACK TX). See section "Transmit and automatically
        // wait for response" in DW1000 User Manual
        if ((status & SYS_STATUS_AAT) && pdw1000local->wait4resp) {
            dwt_forcetrxoff(); // Turn the RX off
            dwt_rxreset(); // Reset in case we were late and a frame was already
                           // being received
        }
...
void dwt_setdelayedtrxtime(uint32 starttime) {
    ...
    dwt_write32bitoffsetreg(DX_TIME_ID, 1,
                            starttime); // Write at offset 1 as the lower 9 bits
 ...
 * - so the receiver will be automatically turned on after TX is done)
 *
 * output parameters
 *
...
int dwt_starttx(uint8 mode) {
    ...
    if (mode & DWT_START_TX_DELAYED) {
               ...
               // individual blocks).
        {
            ...
            // for the specified DX_TIME. Remedial Action - (a) cancel delayed
            // send
            temp = (uint8)
 ...
 * 512/19.2 s (~26.7 s). The counter automatically adds 1 to the value set. The
 * smallest working value that should be set is 1, i.e. giving a snooze time of
 * 2 units (or ~53 s).
 *
 * output parameters
 *
 ...
 * during initialisation (dwt_init). This can be either the value read in OTP
 ...
 */
uint8 dwt_getinitxtaltrim(void) { return pdw1000local->init_xtrim; }
...
uint16 dwt_readtempvbat(uint8 fastSPI) {
    ...
    if (fastSPI == 1) {
        deca_sleep(1); // If using PLL clocks(and fast SPI rate) then this sleep
                       // is needed
    ...
    {
        _dwt_enableclocks(
            FORCE_SYS_XTI); // NOTE: set system clock to XTI - this is necessary
                            // to make sure the values read are reliable
        // Read voltage and temperature.
        dwt_readfromdevice(TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, 2, wr_buf);
        // Default clocks (ENABLE_ALL_SEQ)
        _dwt_enableclocks(ENABLE_ALL_SEQ); // Enable clocks for sequencing
 ...
 *
 * input parameters:
 * @param target_count - uint16 - the PG count target to reach in order to
...
uint32 dwt_calcbandwidthtempadj(uint16 target_count) {
    ...
    // Used to store the current values of the registers so that they can be
    // restored after
    ...
    dwt_write32bitreg(RF_CONF_ID,
                      RF_CONF_TXPOW_MASK | RF_CONF_PGMIXBIASEN_MASK);
    ...
    // Set starting bit
    bit_field = 0x80;
    // Initial lowest delta is the maximum difference that we should allow the
    // count value to be from the target. If the algorithm is successful, it
    // will be overwritten by a smaller value where the count value is closer to
    // the target
    delta_lowest = 300;
    ...
    for (i = 0; i < 7; i++) {
        ...
        raw_count = dwt_read16bitoffsetreg(TX_CAL_ID, TC_PGCAL_STATUS_OFFSET) &
                    TC_PGCAL_STATUS_DELAY_MASK;

        // lets keep track of the closest value to the target in case we
        // overshoot
 ...
 *
 * @brief this function calculates the appropriate change to the TX_POWER
 * register to compensate the TX power output at different temperatures.
 *
 * input parameters:
 * @param ref_powerreg - uint32 - the TX_POWER register value recorded when
 * reference measurements were made
 * @param power_adj - uint32 - the adjustment in power level to be made, in
 * 0.5dB steps
 *
 * output parameters:
 *
...
uint32 _dwt_computetxpowersetting(uint32 ref_powerreg, int32 power_adj) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        while ((current_mixer_gain + mixer_gain_change < 4) ||
            ...
            // If mixer gain goes outside bounds, adjust the DA attenuation to
            // compensate
        ...
        new_regval |= ((uint32)((new_da_attn << 5) | new_mixer_gain))
                      << (i * 8);
    }
 ...
 * setting) for the DW1000 which changes over temperature.
 *
 * input parameters:
 * @param channel - uint8 - the channel at which compensation of power level
 * will be applied
 * @param ref_powerreg - uint32 - the TX_POWER register value recorded when
 * reference measurements were made
 * @param current_temperature - double - the current ambient temperature in
 * degrees Celcius
 * @param reference_temperature - double - the temperature at which reference
 * measurements were made output parameters: None
 *
 * returns: (uint32) The corrected TX_POWER register value
 */
uint32 dwt_calcpowertempadj(uint8 channel, uint32 ref_powerreg,
                            double curr_temp, double ref_temp) {
    ...
    return _dwt_computetxpowersetting(ref_powerreg,
                                      (int32)(delta_power / MIXER_GAIN_STEP));
}
 ...
 *
 * input parameters:
 * @param pgdly - uint8 - the PG_DELAY to set (to control bandwidth), and to
 * find the corresponding count value for output parameters: None
 *
 * returns: (uint16) PGC_STATUS count value calculated from the provided
 * PG_DELAY value - used as reference for later bandwidth adjustments
 */
uint16 dwt_calcpgcount(uint8 pgdly) {

    // Perform PG count read ten times and take an average to smooth out any
    // noise
    ...
    for (i = 0; i < NUM_SAMPLES; i++) {
        ...
        // Allow cal to complete - the TC_PGCCTRL_CALSTART bit will clear
        // automatically
        deca_sleep(100);

        // Read count value from the PG cal block
        count = dwt_read16bitoffsetreg(TX_CAL_ID, TC_PGCAL_STATUS_OFFSET) &
                TC_PGCAL_STATUS_DELAY_MASK;
