...
#define DWT_FF_COORD_EN                                                        \
    0x002 // behave as coordinator (can receive frames with no dest address (PAN
          // ID has to match))
...
typedef struct {
    ...
    uint8 phrMode; //!< PHR mode {0x0 - standard DWT_PHRMODE_STD, 0x3 - extended
 ...
 * @param txFrameBytes   - Pointer to the user\92s buffer containing the data to
 * send.
 * @param txBufferOffset - This specifies an offset in the DW1000\92s TX Buffer
 * at which to start writing data.
 *
 * output parameters
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 ...
 *
 * input parameters:
 * @param mode - is a bitmask for which the following values can be combined to
 * define the operation DWT_START_TX_IMMEDIATE (0)            - to begin
 * transmission immediatelty. DWT_START_TX_DELAYED   (to set bit 0) - to begin
 * TX at pre-configured delay time DWT_RESPONSE_EXPECTED  (to set bit 1) - to
 * turn the receiver on automatically (after the TX) after a pre-programmed
 * delay output parameters
 *
 ...
 * 128/125 \B5s (~1 \B5s). Max value is 255.
 *
 * output parameters
 *
 * no return value
 ...
 * 512/19.2 \B5s (~26.7 \B5s). The counter automatically adds 1 to the value
 * set. The smallest working value that should be set is 1, i.e. giving a snooze
 * time of 2 units (or ~53 \B5s).
 *
 * output parameters
 *
 * no return value
...
void dwt_writetodevice(
    uint16 recordNumber, // input parameter - ID of register file or buffer
                         // being accessed
    uint16 index,  // input parameter - byte index into register file or buffer
                   // being accessed
...
void dwt_readfromdevice(
    ...
    uint8 *buffer  // input parameter - pointer to buffer in which to return the
                   // read data.
);
 ...
 *
 * Description: This function is used to return the range bias correction need
 * for TWR with DW1000 units.
 *
 * input parameters:
 * @param chan  - specifies the operating channel (e.g. 1, 2, 3, 4, 5, 6 or 7)
