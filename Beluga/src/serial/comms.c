...
#define ASCII_FMT_STR_DIAG "%" PRIu16 ",%" PRId8 ",%f,%" PRId64 ",%d,%d,%" PRId64 \
",%f,%f,%f,%f,%f,%f,%f,%f,%" PRIu32 "\r\n"
// TODO: Need to change this to not be floats also
...
//     "{ID:%" PRIu16 ",RSSI:%" PRId8 ",RANGE:%f,TIMESTAMP:%" PRId64                \
//     ",update_flag:%d,polling_flag:%d,ble_time_stamp:%" PRId64                       \
//     ",maxNoise:%f,firstPathAmp1:%f,firstPathAmp2:%f,firstPathAmp3:%f"              \
//     ",stdNoise:%f,maxGrowthCIR:%f,rxPreamCount:%f,firstPath:%f,EXCHANGE:%" PRIu32 "}\r\n"

// #define JSON_FMT_STR_DIAG                                                           \
//     "{ID:%" PRIu16 ",RSSI:%" PRId8 ",RANGE:%f,TIMESTAMP:%" PRId64                \
//     ",update_flag:%" PRIu16 ",polling_flag:%" PRIu16 ",ble_time_stamp:%" PRId64                       \
    ...
    for (size_t i = 0; i < MAX_ANCHOR_COUNT; i++) {
        if (list[i].UUID != 0 && (!stream || list[i].update_flag)) {
            ...
            if (comms->ctx->format == FORMAT_ASCII) {
                ...
                len = snprintf(s, sizeof(s) - 1, ASCII_FMT_STR_DIAG, FMT_PARAMS_DIAG(i));
            } else {
                len = snprintf(s, sizeof(s) - 1, JSON_FMT_STR_DIAG, FMT_PARAMS_DIAG(i));
            }
